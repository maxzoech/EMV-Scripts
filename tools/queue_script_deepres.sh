...
#SBATCH --mem-per-cpu=8G    #	Request 8 GB of memory per CPU core, 16GB total
#SBATCH --gres=gpu:1        #	Request 1 GPU
export OMP_NUM_THREADS=$SLURM_CPUS_PER_TASK


# Initial timestamp
echo "Job with ID $SLURM_JOBID DR-$1-$2 started at $(date)"

#Job steps
eval "$(conda shell.bash hook)"
conda activate scipion3
cmmd="srun --job-name=DR-$1-$2 python /home/bioinfo/services/emv/tools/script_execute_deepres.py -i $1 -p $2"
echo $cmmd
$cmmd

# Final timestamp
echo "Job with ID $SLURM_JOBID DR-$1-$2 ended at $(date)"
