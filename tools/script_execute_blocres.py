...
def create_mask_blocres(map, pdb, sampling, size, path):
    """Convert PDB to Map"""
    ...
    params += " -o %s/align.mrc" % path
    params += " -t vol"
    os.system(params)
    params = " scipion3 run xmipp_image_header"
    params += " -i %s/align.mrc:mrc" % path
    params += " -s %f" % sampling
    os.system(params)
    ...
    params = " scipion3 run xmipp_image_convert"
    params += " -i %s/mask.vol" % path
    params += " -o %s/mask.mrc" % path
    params += " -t vol"
    os.system(params)
    params = " scipion3 run xmipp_image_header"
    params += " -i %s/mask.mrc:mrc" % path
    params += " -s %f" % sampling
    os.system(params)


def execute_blocres(map1, map2, sampling, resolution, cutoff, path):
    ...
    params += " -box %d" % (int(resolution * 6))
    params += " -Mask %s/mask.mrc" % path
    params += " -cutoff %f" % cutoff
    params += " %s/%s" % (path, map1)
    params += " %s/%s" % (path, map2)
    params += " %s/output_blocres.map" % (path)
    os.system(params)


def blocres_to_pdb(pdbId, sampling, org_x, org_y, org_z, path):
    params = "scipion3 run xmipp_pdb_label_from_volume"
    params += " --pdb %s/pdb%s.ent" % (path, pdbId)
    params += " --vol %s/output_blocres.map:mrc" % path
    params += " --mask %s/mask.vol" % path
    params += " --sampling %f" % sampling
    params += " --origin %f %f %f" % (org_x, org_y, org_z)
    params += " -o %s/%s.blocres.atom.pdb" % (path, pdbId)
    os.system(params)


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Compute BlocRes")
    parser.add_argument(
        "-i",
        "--map",
    ...
    create_mask_blocres(inMap, inPdb, sampling, size, workpath)

    print("-> Executing blocres")
    execute_blocres(half1, half2, sampling, resolution, 0.5, workpath)

    print("->  pdb label from blocres map")
    blocres_to_pdb(pdbId, sampling, org_x, org_y, org_z, workpath)
    ...
    convert_to_aapdb(
        pdbId, workpath, cmd="convert_deepres_to_aapdb.py", method="BlocRes"
    )

    print("-> Save in JSON format")
    convert_to_json(
        mapId, pdbId, workpath, cmd="convert_localres_to_json.py", method="BlocRes"
    )
