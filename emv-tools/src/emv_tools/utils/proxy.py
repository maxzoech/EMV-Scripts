...
class Proxy:
    ...
    def __init__(
        self,
        owned=False,
        temp_file_provider: TemporaryFilesProvider = Provide[Container],
    ):
        self.owned = owned
        self.temp_file_provider = temp_file_provider
    ...
    def __del__(self):
        try:
            if self.owned == True:
                self.temp_file_provider.delete(self.path)
        except:
            pass  # Fail silently
...
class TempFileProxy(Proxy):
    ...
    def __init__(self, file_ext=None):
        ...
        self.temp_file = self.temp_file_provider.new_temporary_file(
            suffix
